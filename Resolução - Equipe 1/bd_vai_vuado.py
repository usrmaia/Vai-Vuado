...
class BD:
    ...
    def inserirProduto(self, id_adm_empresa, nome, categoria, medidas, imagem, descricao):
        try:
    ...
    def visualizarProdutosDoAdm(self, id_adm_empresa):
        try:
            self.cursor.execute(f"select * from produtos where id_adm_empresa = '{id_adm_empresa}'")
            return self.cursor.fetchall()
        except sqlite3.Error as erro:
            print(f"Error: {erro}")

    def editarProduto(self, id_produto, id_adm_empresa, nome, categoria, medidas, imagem, descricao):
        try:
    ...
    def deletarProduto(self, id_produto, id_adm_empresa):
        try:
    ...
    def retornarIdAdmEmpresa(self, email, senha):
        try:
            self.cursor.execute(f"select id_adm_empresa from adm_empresa_contratante where email = '{email}' and senha = '{senha}'")
            try:
                return self.cursor.fetchall()[0][0]
            except:
                return False
    ...
    def confirmarSenha(self, id_adm_empresa, senha):
        try:
            self.cursor.execute(f"""
                select true from adm_empresa_contratante where id_adm_empresa = '{id_adm_empresa}' and senha = '{senha}'
            """)
            try:
                return self.cursor.fetchall()[0][0]
            except:
